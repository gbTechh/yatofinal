...
?>
    <main class=" pb-20 hd:pb-32 relative">
      ...
      ?>
      <?php if ($group['titulo']): ?>
        <section class="flex flex-col pt-10 px-5 py-20 hd:p-20 items-center justify-center relative gap-4 bg-gradient-to-br from-white via-gray-50 to-bg-primary/5">
          <div class="relative">
            <img src="<?= $group["imagen"]["url"] ?>" class="rounded-2xl min-h-[150px] object-cover">
            <div class="md:flex bg-bg-primary absolute -left-[10px] hd:-left-10 -bottom-10 rounded-2xl hidden flex-col p-8 max-w-[350px] xl:aspect-square items-left justify-center gap-8">
              <div>
              ...
              <a href="<?= $group["link"] ?>" class="min-w-[150px] text-center text-sm bg-white rounded-full p-2 block w-fit px-5"><?= $group["texto_link"] ?></a>
            </div>
          </div>
          <div class="md:hidden bg-bg-primary  rounded-2xl flex flex-col p-6 w-full  items-left justify-center gap-4">
            ...
            <a href="<?= $group["link"] ?>" class="min-w-[150px] text-center text-xs bg-white rounded-full p-2 block w-fit px-5"><?= $group["texto_link"] ?></a>
          </div>
        </section>
      <?php endif; ?>
      <!--END VIDEO -->
